...
data class Sub(val x: Int, val depth: Int, val aim: Int)
...
object Task1 {
    fun getValue(lines: List<String>): Int {
        ...
        val sub = commands.fold(Sub(0, 0, 0)) { sub, command ->
            when (command.type) {
                Forward -> sub.copy(x = sub.x + command.value)
...
object Task2 {
    fun getValue(lines: List<String>): Int {
        ...
        val sub = commands.fold(Sub(0, 0, 0)) { sub, command ->
            when (command.type) {
                Forward -> sub.copy(x = sub.x + command.value, depth = sub.depth + sub.aim * command.value)
