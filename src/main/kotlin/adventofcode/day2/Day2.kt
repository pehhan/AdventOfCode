...
object Task1 {
    fun numberOfValidPasswords(input: List<String>): Int {
        return input.map { it.split(" ") }.count { isPasswordValid(it[2], it[1][0], it[0].split("-")[0].toInt(), it[0].split("-")[1].toInt()) }
    }
...
object Task2 {
    fun numberOfValidPasswords(input: List<String>): Int {
        return input.map { it.split(" ") }.count { isPasswordValid(it[2], it[1][0], it[0].split("-")[0].toInt() - 1, it[0].split("-")[1].toInt() - 1) }
    }
